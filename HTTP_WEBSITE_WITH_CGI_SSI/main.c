 ...
 * @date 9 May 2023
 * @brief 
 * A main code that able initialize, configuare your WIFI connection and http web-server with ssi and cgi technology
...
#include <stdint.h> /* Use std for correcting choose size of integer value (not just select 'int' value in nay code) */
...
#include "server.h" /* A header file of initialization and run own web server */
/**
 * @brief A variable of uptime since Raspberry Pi PICO W since boot (Connected to power into MCU board) in seconds.
*/
uint64_t current_time_in_seconds = 0;
/**
 * @brief A callback of function that able count digits every one second for showing uptime since powered on board
*/
...
int main() {
    ...
    */
    ...
    /* Run timer function to able count every second since booted Raspberry Pico Pi W */
    ...
    */
    ...
    server_init(); /* Web server initialization. Ready to use after initialization */
